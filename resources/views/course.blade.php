...
<div class="grid grid-cols-3 gap-4">
	<div class="p-8 bg-gray-400 col-span-1">
		<ul class="flex flex-col">				
			<li class="font-medium text-sm text-gray-400 uppercase mb-4">Contenido</li>
                @foreach ($course->posts as $post)
                    <li class="flex items-center text-gray-600 mt-4">
                        {{$post->name}}
                        @if ($post->free)
                            <span class="text-xs text-gray-500 font-semibold bg-gray-300 rounded-full ml-auto p-1 px-2">
                                Gratis
                            </span>
                        @endif
                    </li>
